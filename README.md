...
> **响应速度说明**：一次分析要串行取材 5 组关键词（知乎搜索接口并发会触发限流，只能串行且间隔 ≥400ms），再交给模型做结构化，因此从点击到出结果通常需要十几秒到半分钟。生成模型走的是知乎直答，额度为全租户共享，高峰期会更慢一些。这是取材策略和模型调配决定的，不是页面卡住——过程中会有实时进度提示。
